...
import { AnimatedPage, RevealText } from "@/components/AnimationUtils";
...
export default function BookingPage() {
  ...
  // Simulate some unavailable days (weekends)
      ...
      <div className="min-h-screen pt-28 pb-24 px-6">
        <div className="max-w-2xl mx-auto">
            ...
            {steps.map((s, i) => (
              <div key={s} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  ...
                  />
                  <span className={`font-body text-[10px] uppercase tracking-[0.1em] mt-2 transition-colors duration-300 ${
          ...
          <div className="relative overflow-hidden min-h-[500px]">
            <AnimatePresence mode="wait" custom={dir}>
              ...
              >
                {step === 0 && (
                  <div>
                    <h2 className="font-display text-3xl text-foreground mb-2">Choose your artist</h2>
                ...
                {step === 1 && (
                  <div>
                    <h2 className="font-display text-3xl text-foreground mb-2">Tell us about your tattoo</h2>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {styleOptions.map((s) => (
                            <button
                              key={s}
                              onClick={() => setStyle(s)}
                              className={`font-body text-xs px-5 py-2 border transition-all duration-300 ${
                                style === s ? "bg-foreground text-background border-foreground" : "bg-transparent text-muted-foreground border-border"
                              }`}
                              style={{ borderRadius: 0 }}
                            >
                              {s}
                        ...
                        <div className="flex flex-wrap gap-2">
                          {placementOptions.map((p) => (
                            <button
                              ...
                              className={`font-body text-xs px-5 py-2 border transition-all duration-300 ${
                                placement === p ? "bg-foreground text-background border-foreground" : "bg-transparent text-muted-foreground border-border"
                              }`}
                              style={{ borderRadius: 0 }}
                            >
                              {p}
                        ...
                        <div className="flex flex-wrap gap-2">
                          {sizeOptions.map((s) => (
                            <button
                              ...
                              onClick={() => setSize(s)}
                              className={`font-body text-xs px-5 py-2 border transition-all duration-300 ${
                                size === s ? "bg-foreground text-background border-foreground" : "bg-transparent text-muted-foreground border-border"
                              }`}
                              style={{ borderRadius: 0 }}
                            >
                              {s}
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={4}
                          className="input-dark border border-border w-full p-4 resize-none"
                        />
                      </div>
                    </div>
                  </div>
                ...
                {step === 2 && (
                  <div>
                    <h2 className="font-display text-3xl text-foreground mb-2">Pick a date</h2>
                      ...
                      {days.map((d, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`font-body text-xs py-3 border transition-all duration-300 ${
                              selected
                                ? "bg-foreground text-background border-foreground"
                                : avail
                                  ? "text-foreground border-border hover:border-foreground/50"
                                  : "text-border border-border cursor-not-allowed"
                            }`}
                            style={{ borderRadius: 0 }}
                          >
                            <div className="text-[10px] text-inherit opacity-60">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {timeSlots.map((t) => (
                            <button
                              ...
                              className={`font-body text-sm px-6 py-3 border transition-all duration-300 ${
                                selectedTime === t
                                  ? "bg-foreground text-background border-foreground"
                                  : "text-foreground border-border hover:border-foreground/50"
                              }`}
                ...
                {step === 3 && (
                  <div>
                    <h2 className="font-display text-3xl text-foreground mb-2">Your details</h2>
          ...
          <div className="flex justify-between mt-12">
            {step > 0 ? (
              <button onClick={prev} className="text-link">
                ← Back
              </button>
            ) : <div />}
            {step < 3 ? (
